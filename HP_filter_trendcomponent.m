...
T=length(y); % To find the number of elements in the time series y.
q=1/lambda; % The signal-to-noise ratio
A=[2,-1;1,0]; % State transistion matrix
H=[1,0]; 
Q=[q,0;0,0]; % Process noise covariance (calculated from lambda)
R=1; % Measurement noise covariance (assumed)

g= [2*y(1)-y(2); 3*y(1)-2*y(2)];   % determining {g[t-1]; g[t-2]} by extrapolating 
                                   % to previous time periods using the given time series y(t).
                                   % g(t-1)=y(t)-[y(t+1)-y(t)]= 2*y(t)-y(t+1).
                                   % g(t-2)=y(t)-2*[y(t+1)-y(t)]= 3*y(t)-2*y(t+1).
P= [1000,0;0,1000];                %Assuming initial high values for the covariance
G(1:T)=Zeros;                      %initializing output vector (trend component series)as zeros.
                                   %Preallocation - to reduce the running time.
                                   
 for t=1:T %Get the estimates for each period
       
     g= A*g;        %Time update section
     P= A*P*A'+Q;
     
     K=(P*H')/(H*P*H'+R);   %Measurement update section
     g=g+K*(y(t)-H*g);
     P=(1-K*H)*P;
     
     G(t)=g(1);     % g(1)-> g(t) and g(2)-> g(t-1). Separately saving output series in a separate vector.
 end
end